...
function altcha_admin_page_content()
{
  ...
  wp_localize_script("altcha-admin-wp", "pluginData", array(
    "altcha" => array(
      ...
      "ajaxBaseUrl" => $plugin->normalize_path(wp_parse_url(rest_url("/"), PHP_URL_PATH)),
      "upgradedV2" => $plugin->get_upgraded_v2(),
    ),
  ));
...
function altcha_enqueue_interceptor_scripts()
{
  global $pagenow;
  $plugin = AltchaPlugin::$instance;
  $inject_paths = $plugin->get_settings("injectPaths", array());
  $license = $plugin->get_license();
  ...
  if (!$bypass) {
    switch ($pagenow) {
      case "wp-login.php":
        if (isset($_GET["action"]) && sanitize_text_field($_GET["action"]) === "lostpassword") {
          $should_inject = $plugin->get_settings("protectPasswordReset") === true;
        }else if (isset($_GET["action"]) && sanitize_text_field($_GET["action"]) === "register") {
          $should_inject = $plugin->get_settings("protectRegistration") === true;
        } else {
      ...
      case "wp-comments-post.php":
        $should_inject = $plugin->get_settings("protectComments") === true;
        break;
      default:
        $should_inject = $plugin->match_patterns($plugin->get_request_path(), $inject_paths) !== false;
    }
  }
  $should_inject = apply_filters("altcha_inject", $should_inject, $bypass);
  if ($should_inject) {
    $invisible = !empty($license) && $plugin->get_settings("widgetDisplay") === "invisible";
    wp_enqueue_script(
      "altcha-widget",
    ...
    wp_localize_script("altcha-interceptor-wp", "pluginData", array(
      "altcha" => array(
        ...
        "widget" => array(
          "challenge" => $plugin->get_challenge_url(),
          "delay" => $invisible ? 0 : 1000,
          "display" => $plugin->get_settings("widgetDisplay", "bar"),
          "powAlgorithm" => $plugin->get_settings("powAlgorithm", "PBKDF2/SHA-256"),
        ),
        "invisible" => $invisible,
        "cookiePath" => COOKIEPATH,
        "sitePath" => $plugin->get_site_path(),
        "protectLogin" => $plugin->get_settings("protectLogin", false),
        "protectRegistration" => $plugin->get_settings("protectRegistration", false),
        "protectPasswordReset" => $plugin->get_settings("protectPasswordReset", false),
        "protectComments" => $plugin->get_settings("protectComments", false),
        "underAttack" => $under_attack,
        "underAttackChallengeUrl" => $plugin->get_challenge_url(array(), true),
        "workerArgon2idPath" => plugin_dir_url(__FILE__) . "public/altcha-argon2id.min.js",
      ),
    ));
  }
...
function altcha_enqueue_obfuscation_scripts()
{
  ...
  wp_enqueue_script(
    "altcha-obfuscation",
    plugin_dir_url(__FILE__) . "public/obfuscation.min.js",
    array(),
    AltchaPlugin::$version,
    true
  );
