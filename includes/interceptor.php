...
function altcha_interceptor()
{
  ...
  $protect_registration = $plugin->get_settings("protectRegistration") === true;
  $protect_password_reset = $plugin->get_settings("protectPasswordReset") === true;
  $protect_comments = $plugin->get_settings("protectComments") === true;
  ...
  if ($is_password_reset && $protect_password_reset !== true) {
    // Bypass for password reset page
    return;
  }

  if ($is_registration && $protect_registration !== true) {
    // Bypass for register page
    return;
  }
  ...
  $path_matched = $plugin->match_patterns($plugin->get_request_path(), $paths_list);
  ...
  if ($action === false && $is_registration) {
    $action = "register";
...
function altcha_interceptor_get_field_value(string $name): string
{
  // Parse bracket notation: "group[id]" -> ["group", "id"]
  $keys = preg_split('/[\[\]]+/', rtrim($name, '[]'), -1, PREG_SPLIT_NO_EMPTY);

  $sources = [
    isset($_POST[$keys[0]]) ? $_POST[$keys[0]] : null,
    isset($_GET[$keys[0]])  ? $_GET[$keys[0]]  : null,
  ];
  ...
  foreach ($sources as $value) {
    ...
    // Traverse nested keys for bracket notation
