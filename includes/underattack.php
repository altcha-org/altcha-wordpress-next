...
$pow_algorithm = $plugin->get_settings("powAlgorithm");
...
$argon2id_worker_url = plugin_dir_url(dirname(__FILE__)) . "public/altcha-argon2id.min.js?ver={$version}";
...
$widget_attrs = $plugin->get_widget_attrs($widget_options);
...
echo "<html>
  ...
  <body>
    ...
    <script>
      ...
      function init() {
        if (typeof altchaCreateWorker === 'undefined' && typeof \$altcha === 'undefined') {
          // widget did not load
          return false;
        }
        if (fallbackTimeout) {
        ...
        if (typeof \$altcha !== 'undefined') {
          try {
            \$altcha.algorithms.set('ARGON2ID', () => new Worker('{$argon2id_worker_url}'));
          } catch (err) {
